...
name = strsplit(tex_name, '1');
name = name(1);
imgPath = strcat("../data/", tex_name);
modelPath = strcat("../model/", name, '/');
...
% eval(cmd_fid)
modelName = strcat(modelPath, 'model_', name, '.obj');
fid = fopen(modelName, 'wt');
fprintf(fid, '# obj file\n');
% cmd_print = ['fprintf(fid, ''mtllib model' num2str(im_index) '.mtl\n\n'');'];
% eval(cmd_print);
fprintf(fid, 'mtllib model_', name, '.mtl\n\n');
...
% eval(cmd_mtl)
modelName = strcat(modelPath, 'model_', name, '.mtl');
fid_mtl = fopen(modelName, 'wt');
...
% eval(cmd_fprintf)
fprintf(fid_mtl, 'map_Kd ', tex_name, '\n');
fclose(fid_mtl);
